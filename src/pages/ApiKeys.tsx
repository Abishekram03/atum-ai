    ...
    <div className="flex-1 p-8 h-full flex flex-col overflow-y-auto custom-scrollbar bg-transparent">
        ...
        <header className="mb-8 flex items-start justify-between">
          <div className="flex flex-col">
            <div className="flex items-center gap-3">
              ...
              <span className="bg-[#2A2A2A] text-zinc-400 text-sm px-2.5 py-[1px] rounded-full font-medium">
                {keys?.length || 0}
              </span>
            </div>
            <p className="text-zinc-400 mt-4 text-[15px] font-medium">
              API keys are secure, hashed at rest, and only visible once at
              creation.
            </p>
          </div>
          <button
            ...
            {keys === undefined ? (
              ...
              <div className="p-12 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-zinc-500 text-sm">
                  Create an API key to access Atum from your products.
                </p>
              </div>
            ) : (
              keys.map((k) => (
                <div
                  key={k._id}
                  className="grid grid-cols-[1fr,auto] gap-4 px-6 py-4 items-center border-b border-zinc-800/50 last:border-0 hover:bg-[#2A2A2A]/20 transition-colors"
                >
                  <div className="flex flex-col gap-1">
                    <span className="text-zinc-200 font-medium text-[15px]">
                      {k.name}
                    </span>
                    <span className="text-zinc-500 font-mono text-[13px] tracking-tight">
                      {k.keyPrefix}... {k.isActive ? "active" : "revoked"}
                      {k.product ? ` • ${k.product}` : ""} • used {k.usageCount}
                      x
                    </span>
                  </div>
          ...
          <div className="bg-[#1C1C1E] w-full max-w-md p-6 rounded-2xl shadow-2xl border border-zinc-800">
            ...
            <div className="mb-8">
              ...
              <input
                ...
                placeholder="customer-support-tool"
                className="w-full bg-[#2A2A2A] border border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-zinc-500 transition-colors"
              />
            </div>
      ...
      {createdKey && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#1C1C1E] w-full max-w-2xl p-6 rounded-2xl shadow-2xl border border-zinc-800">
            ...
            <div className="rounded-xl border border-zinc-700 bg-[#2A2A2A] px-4 py-3 font-mono text-zinc-100 text-sm break-all">
              {createdKey}
            </div>
            <div className="mt-5 flex justify-end gap-3">
              <button
                onClick={() => copyToClipboard(createdKey)}
                className="px-4 py-2 rounded-lg bg-zinc-100 text-zinc-900 hover:bg-white transition-colors font-medium text-sm flex items-center gap-2"
              >
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-zinc-300 hover:text-zinc-100 hover:bg-[#2A2A2A] transition-colors font-medium text-sm"
              >
                Done
              </button>
            </div>
